...
# ARG_OPTIONAL_BOOLEAN([copy],,[Copy or Moved?], [on])
...
compose_cmd=(curl -F master_pwd=$PASSWORD -F backup_file=@$_arg_backup)
if [ "$_arg_copy" = "on" ]; then
  compose_cmd+=(-F copy=true)
else
  compose_cmd+=(-F copy=false)
fi

compose_cmd+=(-F name=$_arg_database http://$_arg_server/web/database/restore)
